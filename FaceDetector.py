...
class FaceDetector(object):
    ...
    try:
        ...
        def predict(self, width, height, confidences, boxes, prob_threshold, iou_threshold=0.3, top_k=-1):
            ...
            return self.picked_box_probs[:, :4].astype(np.int32), np.array(
                self.picked_labels), self.picked_box_probs[:, 4]
        ...
        def center_form_to_corner_form(self, locations):
            ...
            return np.concatenate([self.locations[..., :2] - self.locations[..., 2:] / 2,
                                   self.locations[..., :2] + self.locations[..., 2:] / 2],
                                  len(self.locations.shape) - 1)

        def maininfer(self, frame):
            self.frame = frame
            # self.net = dnn.readNetFromONNX(self.onnx_path)  # onnx version
            self.net = cv2.dnn.readNetFromCaffe(self.caffe_prototxt_path,
                                                self.caffe_model_path)  # caffe model converted from onnx
            ...
            self.rect = cv2.resize(self.frame, (self.witdh, self.height))
            ...
            self.boxes = self.convert_locations_to_boxes(self.boxes, self.priors, self.center_variance,
                                                         self.size_variance)
            self.boxes = self.center_form_to_corner_form(self.boxes)
            self.boxes, self.labpythonels, self.probs = self.predict(self.frame.shape[1], self.frame.shape[0],
                                                                     self.scores, self.boxes, self.threshold)
            # for i in range(boxes.shape[0]):
            #     box = boxes[i, :]
            #     cv2.rectangle(frame, (box[0], box[1]), (box[2], box[3]), (0, 255, 0), 2)
            return self.boxes, self.probs

            # image_path = [('file', ('webcam_frame.jpg', img_encoded.tobytes(), 'image/jpeg'))]


    except Exception as e:
        logger.error(f"[FACE DETECTION]: {e}")
